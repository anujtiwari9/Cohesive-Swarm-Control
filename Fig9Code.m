...
beta1_mat = logspace(-4, log(0.95*((beta2_dsr+1) - gamma_dsr*delta_t/2 )/max(lambda_A))/log(10), 10);

velocity_alignment_mat = zeros(3,length(beta1_mat)); %vel coherence for different values of beta1
zeta_mat = zeros(1,length(beta1_mat)); %damping ratio for different values of beta1

for kkk=1:1:length(beta1_mat) 

    kkk/length(beta1_mat)

    beta1_dsr = beta1_mat(kkk);

    zeta_mat(kkk) = ( (beta1_dsr*pi)/(4*gamma_dsr*L) )*( gamma_dsr*a^2/(2*delta_t) );
            ...
            thetadsr = 0*pi/2*ones(num_of_x, 3); %initially the angle is 0
...
loglog(beta1_mat, velocity_alignment_mat(1,:), 'd-', 'MarkerSize', 12, 'LineWidth', 3, 'Color',[0 0 1]);
hold on
loglog(beta1_mat, zeta_mat(1,:), 'o--', 'MarkerSize', 12, 'LineWidth', 3, 'Color',[1 0 0]);
xlabel('Internal damping gain (\beta_1)');
%ylabel('Velocity alignment (v_a)');
%   yticks([0 0.1 0.2:0.2:1])
%  yticklabels({'0','0.1','0.2','0.4','0.6','0.8', '1'})
%   xticks(0.0:0.2:1)
%  xticklabels({'0','0.2','0.4','0.6','0.8','1'})
legend('Velocity alignment (v_a)', 'Damping ratio (\zeta_{0})', 'Location','northwest')
set(gca, 'FontSize', 24);
 saveas(gcf,'Fig_velocity_alignment_and_dampingratio_vs_beta1','epsc')
